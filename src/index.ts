...
export type LeaveForType = Partial<Record<DirectionType, ExtendedSelectorType>>;
...
export interface INavSection extends INavigationConfig {
	...
	getDefaultElement: () => HTMLElement | undefined;
	getLastFocusedElement: () => HTMLElement | undefined;
	getPrimaryElement: () => HTMLElement | undefined;
	getNavigableElements: () => HTMLElement[] | undefined;
...
export default class Navigator {
	...
	public focus(firstArg?: string | boolean, secondArg?: boolean): boolean {
		...
		const autoPause: boolean | undefined = !this.paused && secondArg;
	...
	public move(direction: DirectionType, selector?: string): boolean {
		if (!Reverse[direction.toLowerCase() as DirectionType]) {
			return false;
		}

		const element: HTMLElement | undefined = selector ? selectElements(selector)?.[0] : this.getFocusedElement();

		if (!element) {
			return false;
		}

		const sectionId: string | undefined = this.getSectionId(element);
	...
	private definePriorities(targetRect: IElementRect, direction: DirectionType, groups: IElementRect[][], internalGroups: IElementRect[][], straightOnly: boolean = false): Array<GroupPriorityType | undefined> {
		switch (direction) {
			case Direction.LEFT:
				return [
					{
					...
					straightOnly
						...
						: {
								...
								distanceMeters: [targetRect.nearHorizonIsBetter, targetRect.rightIsBetter, targetRect.nearTargetTopIsBetter],
						  },
				];
				...
				return [
					...
					straightOnly
						...
						: {
								...
								distanceMeters: [targetRect.nearHorizonIsBetter, targetRect.leftIsBetter, targetRect.nearTargetTopIsBetter],
						  },
				];
			case Direction.UP:
				return [
					...
					straightOnly
						...
						: {
								...
								distanceMeters: [targetRect.nearPlumbLineIsBetter, targetRect.bottomIsBetter, targetRect.nearTargetLeftIsBetter],
						  },
				];
			case Direction.DOWN:
				return [
					...
					straightOnly
						...
						: {
								...
								distanceMeters: [targetRect.nearPlumbLineIsBetter, targetRect.topIsBetter, targetRect.nearTargetLeftIsBetter],
						  },
				];
			default:
				return [];
	...
	private prioritize(priorities: Array<GroupPriorityType | undefined>): IElementRect[] {
	...
	private navigate(target: HTMLElement, direction: DirectionType, candidates: HTMLElement[], config: NavConfigType = Navigator.config): HTMLElement | undefined {
		...
		const destinationGroup: IElementRect[] = this.prioritize(this.definePriorities(targetRect, direction, groups, internalGroups, config.straightOnly));
		...
		if (config.rememberSource && config.previousFocus && config.previousFocus.destination === target && config.previousFocus.reverse === direction) {
	...
	private getFocusedElement(): HTMLElement | undefined {
	...
	public focusElement(element: HTMLElement, sectionId?: string, direction?: DirectionType): boolean {
		...
		const focusedElement: HTMLElement | undefined = this.getFocusedElement();
	...
	public focusExtendedSelector(selector: ExtendedSelectorType, direction?: DirectionType): boolean {
		if ("string" === typeof selector && "@" === selector.charAt(0)) {
			...
			const element: HTMLElement | undefined = selectElements(selector)?.[0];
	...
	private focusSection(sectionId?: string): boolean {
		...
		const addRange: (id: string) => void = (id: string) => {
			if (id && !range.includes(id) && this.sections[id] && !this.sections[id].isDisabled()) {
				range.push(id);
			}
		};
	...
	private focusNext(direction: DirectionType, focusedElement: HTMLElement, currentSectionId: string): boolean {
		const extSelector: string | null = focusedElement.getAttribute("data-sn-" + direction);
		...
		for (const id in this.sections) {
			const collection: HTMLElement[] | undefined = this.sections[id].getNavigableElements();

			if (collection) {
				sectionsNavigableElements[id] = collection;
		...
		if (config.restrict === Restrict.SELF_ONLY || config.restrict === Restrict.SELF_FIRST) {
			...
			nextElement = this.navigate(focusedElement, direction, exclude(currentSectionNavigableElements, focusedElement), config);

			if (!nextElement && config.restrict === Restrict.SELF_FIRST) {
				nextElement = this.navigate(focusedElement, direction, exclude(allNavigableElements, currentSectionNavigableElements), config);
			}
		} else {
		...
		if (nextElement) {
			...
			const nextSectionId: string | undefined = this.getSectionId(nextElement);
			...
			if (currentSectionId !== nextSectionId) {
				...
				const enterToElement: HTMLElement | undefined = nextSectionId ? this.sections[nextSectionId].getPrimaryElement() : undefined;
	...
	private onKeyDown(event: KeyboardEvent): boolean {
		...
		const sectionId: string | undefined = this.getSectionId(focusedElement);
	...
	private onFocus({ target }: FocusEvent): void {
		if (window === target || document === target) {
		...
		if (target !== window && target !== document && this.sectionCount && !this.duringFocusChange) {
			const sectionId: string | undefined = this.getSectionId(target as HTMLElement);
	...
	private onBlur({ target }: FocusEvent): void {
		if (target !== window && target !== document && !this.paused && this.sectionCount && !this.duringFocusChange && this.getSectionId(target as HTMLElement)) {
