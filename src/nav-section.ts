...
export default class NavSection implements INavSection {
  ...
  public previousFocus?: PreviousFocus = undefined;
  ...
  constructor(
    navigator: Navigator,
    config: Omit<NavConfig, 'lastFocusedElement' | 'previousFocus'>,
    id?: string,
  ) {
    if (navigator) {
  ...
  public setConfig(config: NavConfig): void {
  ...
  public savePreviousFocus(target: HTMLElement, destination: HTMLElement, reverse: Direction): void {
  ...
  private isCallback(leaveFor: ExtendedSelector | (() => ExtendedSelector)): leaveFor is (() => ExtendedSelector) {
    return leaveFor instanceof Function;
  }

  private getLeaveForAt(direction: Direction): ExtendedSelector {
    if (this.leaveFor && this.leaveFor[direction] !== undefined) {
      if (this.isCallback(this.leaveFor[direction])) {
  ...
  public gotoLeaveFor(direction: Direction): boolean | undefined {
    const selector: ExtendedSelector = this.getLeaveForAt(direction);
